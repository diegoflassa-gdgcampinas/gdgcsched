...
public class SessionDetailActivity_OnGoingLiveStarredSessionTest {
    ...
    public void sessionTitle_ShowsCorrectTitle() {
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.session_title)).check(matches(
                allOf(withText(SessionsMockCursor.FAKE_TITLE), isDisplayed())));
    }

    @Test
    public void liveStreamedText_IsVisible() {
        onView(ViewMatchers.withText(org.gdg_campinas.treffen.lib.R.string.session_live_streamed)).check(matches(isDisplayed()));
    }

    @Test
    public void watchLiveText_IsVisible() {
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.watch)).check(matches(isDisplayed()));
        onView(ViewMatchers.withText(org.gdg_campinas.treffen.lib.R.string.session_watch_live)).check(matches(isDisplayed()));
    }

    @Test
    public void headerImage_IsVisible() {
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.session_photo)).check(matches(isDisplayed()));
    }

    @Test
    public void speakersSection_IsVisible() {
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.session_detail_frag)).perform(swipeUp());
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.session_speakers_block)).check(matches(isDisplayed()));
    }

    @Test
    public void tagSection_IsVisible() {
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.session_tags_container)).check(matches(withEffectiveVisibility(
                ViewMatchers.Visibility.VISIBLE)));
    }

    @Test
    public void feedbackButton_IsNotVisible() {
        onView(ViewMatchers.withId(org.gdg_campinas.treffen.lib.R.id.give_feedback_button)).check(matches(not(isDisplayed())));
    }
