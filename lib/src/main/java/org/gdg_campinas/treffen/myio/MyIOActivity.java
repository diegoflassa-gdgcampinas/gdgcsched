...
import org.gdg_campinas.treffen.schedule.ScheduleView;
import org.gdg_campinas.treffen.signin.RegistrationStatusService;
...
public class MyIOActivity extends BaseActivity implements
    ...
    private static final String TAG = LogUtils.makeLogTag(MyIOActivity.class);
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        mValueEventListener = new ValueEventListener() {
            ...
            public void onCancelled(DatabaseError error) {
                LogUtils.LOGW(TAG, "Listener was cancelled");
            }
        };
    }
    ...
    protected void onStart() {
        ...
        LogUtils.LOGI(TAG, "Time since last reg check:" + timeSinceLastRegCheck);
        LogUtils.LOGI(TAG, "Status:" + RegistrationUtils.isRegisteredAttendee(this));
    ...
    public void onConnected(@Nullable final Bundle bundle) {
        LogUtils.LOGD(TAG, "GoogleApiClient connected");
        if (mSignOutPending) {
            mSignInManager.signOut();
    ...
    public void onConnectionSuspended(final int i) {
        LogUtils.LOGW(TAG, "GoogleApiClient suspended");
    }
    ...
    public void onSignInFailed(GoogleSignInResult result) {
        if(result != null) {
            LogUtils.LOGW(TAG, "Failed to sign in: status code == " + result.getStatus().getStatusCode());
        }
    ...
    private void showAnnouncementDialogIfNeeded(Intent intent) {
        ...
        if (!TextUtils.isEmpty(title) && !TextUtils.isEmpty(message)) {
            ...
            LogUtils.LOGD(TAG, String.format(
