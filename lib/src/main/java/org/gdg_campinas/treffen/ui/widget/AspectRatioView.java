...
public class AspectRatioView extends FrameLayout {
    ...
    public AspectRatioView(Context context, AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs,
                org.gdg_campinas.treffen.lib.R.styleable.AspectRatioView, defStyle, 0);
        mAspectRatio = a.getFloat(org.gdg_campinas.treffen.lib.R.styleable.AspectRatioView_aspectRatio, 0);
        if (mAspectRatio == 0f) {
            throw new IllegalArgumentException(
                    "You must specify an aspect ratio when using AspectRatioView.");
        }
        mMaxHeight = a.getDimensionPixelSize(org.gdg_campinas.treffen.lib.R.styleable.AspectRatioView_maxHeight, NO_MAX_HEIGHT);
        a.recycle();
    }
