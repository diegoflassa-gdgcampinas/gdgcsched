...
package org.gdg_campinas.treffen.ui.widget;
...
import org.gdg_campinas.treffen.util.AccessibilityUtils;
...
public class CustomRatingBar extends LinearLayout implements View.OnClickListener {
     ...
     */
    private static final int DEFAULT_UNFILLED_DRAWABLE_ID = org.gdg_campinas.treffen.lib.R.drawable.ic_star_empty;

    /**
     * The drawable used if no filled drawable resource is specified.
     */
    private static final int DEFAULT_FILLED_DRAWABLE_ID = org.gdg_campinas.treffen.lib.R.drawable.ic_star_filled;
    ...
    public CustomRatingBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(
                attrs,
                org.gdg_campinas.treffen.lib.R.styleable.CustomRatingBar,
                0, 0);

        try {
            mMaxRating = typedArray.getInt(org.gdg_campinas.treffen.lib.R.styleable.CustomRatingBar_maxRating,
                    DEFAULT_MAX_RATING);
            mFilledDrawable = typedArray.getDrawable(org.gdg_campinas.treffen.lib.R.styleable.CustomRatingBar_filledDrawable);
            if (mFilledDrawable == null) {
                mFilledDrawable = ResourcesCompat.getDrawable(getResources(),
                        DEFAULT_FILLED_DRAWABLE_ID, null);
            }
            mUnfilledDrawable = typedArray.getDrawable(org.gdg_campinas.treffen.lib.R.styleable.CustomRatingBar_unfilledDrawable);
    ...
    public void sendAccessibilityEvent(final int eventType) {
        ...
        event.getText().add(getContext().getResources().getString(
                org.gdg_campinas.treffen.lib.R.string.feedback_rating_confirmation, mRating, mMaxRating));
    ...
    private void createRatingViews() {
        for (int i = 0; i < mMaxRating; i++) {
            ...
            imageView.setContentDescription(getContext().getString(
                    org.gdg_campinas.treffen.lib.R.string.feedback_rating_value, tagValue));
