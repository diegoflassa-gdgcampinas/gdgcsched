...
public class InfoViewPagerAdapter extends FragmentPagerAdapter {
    ...
    private final static String TAG = LogUtils.makeLogTag(InfoViewPagerAdapter.class);
    ...
    public BaseInfoFragment getItem(int position) {
        LogUtils.LOGD(TAG, "Creating BaseInfoFragment #" + position);
    ...
    public void setRetainedFragmentsTags(String[] tags) {
        if (tags != null && tags.length > 0) {
            ...
            for (int i = 0; i < tags.length; i++) {
                ...
                if (fragment == null) {
                    LogUtils.LOGE(TAG, "Fragment with existing tag " + tags[i] + " not found!");
