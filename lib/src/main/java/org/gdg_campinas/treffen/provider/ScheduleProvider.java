...
import org.gdg_campinas.treffen.util.SelectionBuilder;
import org.gdg_campinas.treffen.appwidget.ScheduleWidgetProvider;
import org.gdg_campinas.treffen.provider.ScheduleContract.Announcements;
import org.gdg_campinas.treffen.provider.ScheduleContract.Blocks;
import org.gdg_campinas.treffen.provider.ScheduleContract.Feedback;
import org.gdg_campinas.treffen.provider.ScheduleContract.HashtagColumns;
import org.gdg_campinas.treffen.provider.ScheduleContract.Hashtags;
import org.gdg_campinas.treffen.provider.ScheduleContract.MyFeedbackSubmitted;
import org.gdg_campinas.treffen.provider.ScheduleContract.MyReservationColumns;
import org.gdg_campinas.treffen.provider.ScheduleContract.MyReservations;
import org.gdg_campinas.treffen.provider.ScheduleContract.MySchedule;
import org.gdg_campinas.treffen.provider.ScheduleContract.MyScheduleColumns;
import org.gdg_campinas.treffen.provider.ScheduleContract.MyViewedVideos;
import org.gdg_campinas.treffen.provider.ScheduleContract.Rooms;
import org.gdg_campinas.treffen.provider.ScheduleContract.SearchSuggest;
import org.gdg_campinas.treffen.provider.ScheduleContract.SearchTopicsSessions;
import org.gdg_campinas.treffen.provider.ScheduleContract.Sessions;
import org.gdg_campinas.treffen.provider.ScheduleContract.Speakers;
import org.gdg_campinas.treffen.provider.ScheduleContract.Tags;
import org.gdg_campinas.treffen.provider.ScheduleContract.VideoColumns;
import org.gdg_campinas.treffen.provider.ScheduleContract.Videos;
...
public class ScheduleProvider extends ContentProvider {
        ...
        if (requiredTags.length == 0) {
            ...
            if (numCategories != null && TextUtils.isDigitsOnly(numCategories)) {
                try {
                    ...
                    LogUtils.LOGD(TAG, "Categories being used " + categories);
                } catch (Exception ex) {
                    LogUtils.LOGE(TAG, "exception parsing categories ", ex);
                }
            }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        if (Log.isLoggable(TAG, Log.VERBOSE)) {
            LogUtils.LOGV(TAG, "uri=" + uri + " code=" + matchingUriEnum.code + " proj=" +
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (matchingUriEnum) {
            ...
            case SESSIONS_ID_SPEAKERS: {
                return Speakers.buildSpeakerUri(values.getAsString(ScheduleDatabase.SessionsSpeakers.SPEAKER_ID));
            }
            ...
            case SESSIONS_ID_RELATED: {
                ...
                db.insertOrThrow(ScheduleDatabase.Tables.RELATED_SESSIONS, null, values);
                notifyChange(uri);
                return uri;
            }
            case MY_SCHEDULE: {
                values.put(MySchedule.MY_SCHEDULE_ACCOUNT_NAME, getCurrentAccountName(uri, false));
                db.insertOrThrow(ScheduleDatabase.Tables.MY_SCHEDULE, null, values);
                notifyChange(uri);
            ...
            case MY_RESERVATIONS: {
                ...
                db.insertOrThrow(ScheduleDatabase.Tables.MY_RESERVATIONS, null, values);
                notifyChange(uri);
                Uri sessionUri = Sessions.buildSessionUri(
            ...
            case MY_VIEWED_VIDEOS: {
                ...
                db.insertOrThrow(ScheduleDatabase.Tables.MY_VIEWED_VIDEO, null, values);
                notifyChange(uri);
            ...
            case MY_FEEDBACK_SUBMITTED: {
                values.put(MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_ACCOUNT_NAME,
                        getCurrentAccountName(uri, false));
                db.insertOrThrow(ScheduleDatabase.Tables.MY_FEEDBACK_SUBMITTED, null, values);
                notifyChange(uri);
                Uri sessionUri = Sessions.buildSessionUri(
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        String accountName = getCurrentAccountName(uri, false);
        LogUtils.LOGV(TAG, "delete(uri=" + uri + ", account=" + accountName + ")");
    ...
    private SelectionBuilder buildSimpleSelection(Uri uri) {
        ...
        switch (matchingUriEnum) {
            ...
            case TAGS_ID: {
                ...
                return builder.table(ScheduleDatabase.Tables.TAGS)
            ...
            case SESSIONS_ID_TAGS: {
                final String sessionId = Sessions.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_TAGS)
                        .where(Sessions.SESSION_ID + "=?", sessionId);
            }
            case SESSIONS_ID_RELATED: {
                final String sessionId = Sessions.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.RELATED_SESSIONS)
                        .where(Sessions.SESSION_ID + "=?", sessionId);
            }
            ...
            case MY_RESERVATIONS: {
                return builder.table(ScheduleDatabase.Tables.MY_RESERVATIONS)
                        .where(MyReservations.MY_RESERVATION_ACCOUNT_NAME + "=?",
                                getCurrentAccountName(uri, false));
            }
            case MY_VIEWED_VIDEOS: {
                return builder.table(ScheduleDatabase.Tables.MY_VIEWED_VIDEO)
                        .where(MyViewedVideos.MY_VIEWED_VIDEOS_ACCOUNT_NAME + "=?",
                                getCurrentAccountName(uri, false));
            }
            case MY_FEEDBACK_SUBMITTED: {
                return builder.table(ScheduleDatabase.Tables.MY_FEEDBACK_SUBMITTED)
            ...
            case ANNOUNCEMENTS_ID: {
                ...
                return builder.table(ScheduleDatabase.Tables.ANNOUNCEMENTS)
                        .where(Announcements.ANNOUNCEMENT_ID + "=?", announcementId);
            }
            case FEEDBACK_FOR_SESSION: {
                final String session_id = Feedback.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.FEEDBACK)
            ...
            case HASHTAGS_NAME: {
                ...
                return builder.table(ScheduleDatabase.Tables.HASHTAGS)
                        .where(Hashtags.HASHTAG_NAME + "=?", hashtagName);
            }
            case VIDEOS_ID: {
                final String videoId = Videos.getVideoId(uri);
                return builder.table(ScheduleDatabase.Tables.VIDEOS).where(Videos.VIDEO_ID + "=?", videoId);
            }
            default: {
    ...
    private SelectionBuilder buildExpandedSelection(Uri uri, int match) {
        ...
        switch (matchingUriEnum) {
            ...
            case CARDS: {
                return builder.table(ScheduleDatabase.Tables.CARDS);
            }
            case TAGS: {
                return builder.table(ScheduleDatabase.Tables.TAGS);
            }
            ...
            case ROOMS_ID_SESSIONS: {
                final String roomId = Rooms.getRoomId(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS, accountName, accountName)
            ...
            case SESSIONS: {
                ...
                // similarly.
                return builder
                        .table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS, accountName, accountName)
                        .mapToTable(Sessions._ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(Sessions.ROOM_ID, ScheduleDatabase.Tables.SESSIONS)
            ...
            case SESSIONS_COUNTER: {
                return builder
                        .table(ScheduleDatabase.Tables.SESSIONS_JOIN_MYSCHEDULE, accountName, accountName)
            ...
            case SESSIONS_MY_SCHEDULE: {
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS_FEEDBACK_MYSCHEDULE,
            ...
            case SESSIONS_SEARCH: {
                ...
                return builder.table(ScheduleDatabase.Tables.SESSIONS_SEARCH_JOIN_SESSIONS_ROOMS,
                        accountName, accountName)
            ...
            case SESSIONS_ID_SPEAKERS: {
                ...
                return builder.table(ScheduleDatabase.Tables.SESSIONS_SPEAKERS_JOIN_SPEAKERS)
                        .mapToTable(Speakers._ID, ScheduleDatabase.Tables.SPEAKERS)
                        .mapToTable(Speakers.SPEAKER_ID, ScheduleDatabase.Tables.SPEAKERS)
                        .where(Qualified.SESSIONS_SPEAKERS_SESSION_ID + "=?", sessionId);
            }
            case SESSIONS_ID_TAGS: {
                final String sessionId = Sessions.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_TAGS_JOIN_TAGS)
                        .mapToTable(Tags._ID, ScheduleDatabase.Tables.TAGS)
                        .mapToTable(Tags.TAG_ID, ScheduleDatabase.Tables.TAGS)
            ...
            case SESSIONS_ROOM_AFTER: {
                ...
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS, accountName, accountName)
                        .mapToTable(Sessions._ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(Sessions.ROOM_ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(Sessions.SESSION_ID, ScheduleDatabase.Tables.SESSIONS)
            ...
            case SPEAKERS_ID_SESSIONS: {
                final String speakerId = Speakers.getSpeakerId(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_SPEAKERS_JOIN_SESSIONS_ROOMS,
                        accountName, accountName)
                        .mapToTable(Sessions._ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(Sessions.SESSION_ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(Sessions.ROOM_ID, ScheduleDatabase.Tables.SESSIONS)
            ...
            case MAPGEOJSON: {
                return builder.table(ScheduleDatabase.Tables.MAPGEOJSON);
            }
            case MAPTILES: {
                return builder.table(ScheduleDatabase.Tables.MAPTILES);
            }
            case FEEDBACK_FOR_SESSION: {
            ...
            case VIDEOS: {
                return builder.table(ScheduleDatabase.Tables.VIDEOS);
            }
            case VIDEOS_ID: {
                final String videoId = Videos.getVideoId(uri);
                return builder.table(ScheduleDatabase.Tables.VIDEOS)
    ...
    private interface Subquery {
        String SESSION_HAS_GIVEN_FEEDBACK = "(SELECT COUNT(1) FROM "
                + ScheduleDatabase.Tables.FEEDBACK + " WHERE " + Qualified.FEEDBACK_SESSION_ID + "="
                + Qualified.SESSIONS_SESSION_ID + ")";

        String SESSIONS_SNIPPET = "snippet(" + ScheduleDatabase.Tables.SESSIONS_SEARCH + ",'{','}','\u2026')";

        String RELATED_SESSIONS_SELECTION = Qualified.SESSIONS_SESSION_ID + " IN (SELECT "
                + Sessions.RELATED_SESSION_ID + " FROM " + ScheduleDatabase.Tables.RELATED_SESSIONS + " WHERE "
                + Sessions.SESSION_ID + " = ?)";
    }
    ...
    private interface Qualified {
        String SESSIONS_SESSION_ID = ScheduleDatabase.Tables.SESSIONS + "." + Sessions.SESSION_ID;
        String SESSIONS_ROOM_ID = ScheduleDatabase.Tables.SESSIONS + "." + Sessions.ROOM_ID;
        String SESSIONS_TAGS_SESSION_ID = ScheduleDatabase.Tables.SESSIONS_TAGS + "."
                + ScheduleDatabase.SessionsTags.SESSION_ID;

        String SESSIONS_SPEAKERS_SESSION_ID = ScheduleDatabase.Tables.SESSIONS_SPEAKERS + "."
                + ScheduleDatabase.SessionsSpeakers.SESSION_ID;

        String SESSIONS_SPEAKERS_SPEAKER_ID = ScheduleDatabase.Tables.SESSIONS_SPEAKERS + "."
                + ScheduleDatabase.SessionsSpeakers.SPEAKER_ID;

        String FEEDBACK_SESSION_ID = ScheduleDatabase.Tables.FEEDBACK + "." + Feedback.SESSION_ID;
    }
}
