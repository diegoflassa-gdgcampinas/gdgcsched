...
import org.gdg_campinas.treffen.provider.ScheduleProvider;
 ...
 * data from the {@link ScheduleProvider}.
 */
public abstract class ModelWithLoaderManager<Q extends QueryEnum, UA extends UserActionEnum>
    ...
    private static final String TAG = LogUtils.makeLogTag(ModelWithLoaderManager.class);
    /**
     * Map of callbacks, using the query as key. This is required because we can't pass on the
     * {@link Model.DataQueryCallback} to the {@link
    ...
    public void deliverUserAction(@NonNull UA action, @Nullable Bundle args,
        ...
        PreconditionUtils.checkNotNull(callback);
        PreconditionUtils.checkNotNull(action);
        if (args != null && args.containsKey(KEY_RUN_QUERY_ID)) {
            ...
            if (queryId instanceof Integer) {
                if (isQueryValid((Integer) queryId) && mLoaderManager != null) {
                    ...
                    LogUtils.LOGE(TAG, "onUserAction called with a bundle containing KEY_RUN_QUERY_ID but"
