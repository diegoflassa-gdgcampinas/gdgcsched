...
public class RegistrationStatusService extends Service {
    private static final String TAG = LogUtils.makeLogTag(RegistrationStatusService.class);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        LogUtils.LOGD(TAG, "Refreshing registration status...");
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    LogUtils.LOGE(TAG, "Unable to update registration status", e);
                } finally {
                    stopSelf();
                }
            }
    ...
    private void updateRegStatus() throws Exception {
        ...
        LogUtils.LOGD(TAG, "Firebase token: " + fbToken);
        LogUtils.LOGD(TAG, "Authenticating as: " + credential.getSelectedAccountName());

        // Communicate with server
        boolean isRegistered;
        isRegistered = isRegisteredAttendee(credential, fbToken);
        LogUtils.LOGD(TAG, "Conference attendance status: " +
