...
public class SessionCalendarService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (!permissionsAlreadyGranted()) {
            LogUtils.LOGW(TAG, "Calendar permission not granted.");
            return;
        }

        final String action = intent.getAction();
        LogUtils.LOGD(TAG, "Received intent: " + action);
    ...
    private String makeCalendarEventTitle(String sessionTitle) {
        return sessionTitle + getResources().getString(org.gdg_campinas.treffen.lib.R.string.session_calendar_suffix);
    }
