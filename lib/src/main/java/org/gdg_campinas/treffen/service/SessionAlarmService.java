...
public class SessionAlarmService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_NOTIFY_SESSION.equals(action)) {
            LogUtils.LOGD(TAG, "Notifying about sessions starting at " +
            ...
            LogUtils.LOGD(TAG, "-> Session start: " + sessionStart + " = " + (new Date(sessionStart))
                    .toString());
        ...
        try {
            ...
            while (c.moveToNext()) {
                ...
                if (feedbackHelper.isFeedbackNotificationFiredForSession(sessionId)) {
                    LogUtils.LOGD(TAG, "Skipping repeated session feedback notification for session '"
                            + sessionTitle + "'");
                    continue;
                }
            ...
            LogUtils.LOGD(TAG, "Going forward with session feedback notification for "
