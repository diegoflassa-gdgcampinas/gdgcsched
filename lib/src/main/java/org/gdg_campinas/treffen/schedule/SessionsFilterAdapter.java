...
public class SessionsFilterAdapter extends Adapter<ViewHolder> {
    ...
    private void buildFiltersList(TagMetadata tagMetadata) {
        ...
        // Types
        List<TagMetadata.Tag> types = tagMetadata.getTagsInCategory(Config.Tags.CATEGORY_TYPE);
    ...
    void addTag(TagMetadata.Tag tag) {
    ...
    public ViewHolder onCreateViewHolder(final ViewGroup parent, final int viewType) {
        switch (viewType) {
            ...
            default:
                LogUtils.LOGE(TAG, "Unknown view type");
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        switch (holder.getItemViewType()) {
            ...
            case TYPE_TOPIC:
                ...
                TagMetadata.Tag filter = (TagMetadata.Tag) mItems.get(position);
    ...
    public int getItemViewType(final int position) {
        ...
        if (item instanceof TopicsHeader) {
            return TYPE_HEADER;
        } else if (item instanceof TagMetadata.Tag) {
            TagMetadata.Tag tag = (TagMetadata.Tag) item;
            if (Config.Tags.CATEGORY_TYPE.equals(tag.getCategory())) {
                return TYPE_TYPE;
            }
            return TYPE_TOPIC;
        }
    ...
    private void updateTagFilter(TagMetadata.Tag filter, boolean checked) {
    ...
    private class FilterViewHolder extends ViewHolder implements OnCheckedChangeListener, DividerDecoration.Divided {

        final TextView mLabel;
        final CheckBox mCheckbox;
        private @Nullable
        TagMetadata.Tag mTagFilter;
        ...
        void bind(TagMetadata.Tag filter, boolean checked) {
            mTagFilter = filter;

            mLabel.setText(filter.getName());
            mCheckbox.setContentDescription(mLabel.getText());
            setChecked(checked);
            if (filter.getId().startsWith(Config.Tags.CATEGORY_TRACK)) {
