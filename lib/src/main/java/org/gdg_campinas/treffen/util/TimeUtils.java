...
public class TimeUtils {
    ...
    public static String formatDuration(@NonNull Context context, long duration) {
        ...
        if (hours >= 1f) {
            return context.getResources().getQuantityString(org.gdg_campinas.treffen.lib.R.plurals.duration_hours,
                    (int) Math.ceil(hours), (hours == hours.intValue()) ?
                            String.valueOf(hours.intValue()) : hours.toString());
        } else {
            long mins = duration / MINUTE;
            return context.getResources().getQuantityString(
                    org.gdg_campinas.treffen.lib.R.plurals.duration_minutes, (int) mins, mins);
        }
    }
    ...
    public static String formatHumanFriendlyShortDate(final Context context, long timestamp) {
        ...
        if (dayOrd == nowOrd) {
            return context.getString(org.gdg_campinas.treffen.lib.R.string.day_title_today);
        } else if (dayOrd == nowOrd - 1) {
            return context.getString(org.gdg_campinas.treffen.lib.R.string.day_title_yesterday);
        } else if (dayOrd == nowOrd + 1) {
            return context.getString(org.gdg_campinas.treffen.lib.R.string.day_title_tomorrow);
        } else {
    ...
    public static void setAppStartTime(Context context, long newTime) {
        if (BuildConfig.DEBUG) {
            java.util.Date previousAppStartTime = new java.util.Date(TimeUtils.getAppStartTime(
                    context));
            LogUtils.LOGW(TAG, "Setting app startTime from " + previousAppStartTime + " to " + newTime);
            context.getSharedPreferences(UIUtils.MOCK_DATA_PREFERENCES, Context.MODE_PRIVATE).edit()
