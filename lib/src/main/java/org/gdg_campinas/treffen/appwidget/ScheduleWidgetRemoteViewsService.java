...
import org.gdg_campinas.treffen.ui.SimpleSectionedListAdapter;
    ...
    private static class WidgetRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
        ...
        public int getItemViewType(int position) {
            if (position < 0 || position >= mScheduleItems.size()) {
                LogUtils.LOGE(TAG, "Invalid view position passed to ScheduleDayAdapter: " + position);
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (isSectionHeader) {
                ...
                if (item.type == ScheduleItem.FREE) {
                    ...
                    LogUtils.LOGE(TAG, "Invalid item type in ScheduleDayAdapter: " + item.type);
                }
            }
