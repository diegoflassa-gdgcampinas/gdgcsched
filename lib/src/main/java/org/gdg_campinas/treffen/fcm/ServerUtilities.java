...
public final class ServerUtilities {
    ...
    private static boolean checkFcmEnabled() {
        ...
        if (TextUtils.isEmpty(BuildConfig.FCM_SERVER_URL)) {
            LogUtils.LOGD(TAG, "FCM feature disabled (no URL configured)");
            return false;
        } else if (TextUtils.isEmpty(BuildConfig.FCM_API_KEY)) {
            LogUtils.LOGD(TAG, "FCM feature disabled (no API key configured)");
            return false;
        } else if (TextUtils.isEmpty(BuildConfig.FCM_SENDER_ID)) {
            LogUtils.LOGD(TAG, "FCM feature disabled (no sender ID configured)");
            return false;
        }
        return true;
    }
    ...
    public static boolean register(final Context context, final String deviceId,
        ...
        try {
            LogUtils.LOGD(TAG, "registering device (reg_id = " + deviceId + ")");
            getFcmHandler(context).fcmRegistrationEndpoint().register(deviceId).execute();
            LogUtils.LOGI(TAG, "registering on FCM with FCM key: " + AccountUtils.sanitizeUserId(userId));
            return true;
        } catch (IOException e) {
            e.printStackTrace();
            LogUtils.LOGI(TAG, "Failed to register with user ID: " + userId + " and device ID: " + deviceId);
            return false;
        }
    }
    ...
    public static void unregister(final Context context, final String deviceId) {
        ...
        LogUtils.LOGI(TAG, "unregistering device (deviceId = " + deviceId + ")");
        if (AccountUtils.hasActiveAccount(context) && deviceId != null) {
            ...
            LogUtils.LOGD(TAG, "User must be signed in to unregister device and device ID must be non null");
        }
    }
    ...
    public static void notifyUserDataChanged(final Context context) {
        ...
        LogUtils.LOGI(TAG, "Notifying FCM that user data changed");
        try {
            if (AccountUtils.hasActiveAccount(context)) {
                getPingHandler(context).sendSelfSync().execute();
            }
        } catch (IOException e) {
            LogUtils.LOGE(TAG, "Unable to notify FCM about user data change", e);
        }
    }
    ...
    static void setRegisteredOnServer(Context context, boolean flag, String deviceId,
        ...
        LogUtils.LOGD(TAG, "Setting registered on server status as: " + flag + ", fcmKey="
    ...
    static boolean isRegisteredOnServer(Context context, String userId) {
        ...
        if (regTS > yesterdayTS) {
            LogUtils.LOGV(TAG, "FCM registration current. regTS=" + regTS + " yesterdayTS=" + yesterdayTS);

            final String registeredUserId = prefs.getString(KEY_USER_ID, "");
            if (registeredUserId.equals(userId)) {
                LogUtils.LOGD(TAG, "FCM registration is valid and for the correct fcm key: "
                        + AccountUtils.sanitizeUserId(registeredUserId));
                return true;
            }
            LogUtils.LOGD(TAG, "FCM registration is for DIFFERENT FCM key "
                    + AccountUtils.sanitizeUserId(registeredUserId) + ". We were expecting "
                    + AccountUtils.sanitizeUserId(userId));
            return false;
        } else {
            LogUtils.LOGV(TAG, "FCM registration expired. regTS=" + regTS + " yesterdayTS=" + yesterdayTS);
            return false;
        }
    }
    ...
    private static void post(String endpoint, Map<String, String> params, String key)
        ...
        LogUtils.LOGW(TAG, "Posting to " + url);
        LogUtils.LOGV(TAG, "Posting '" + body + "'");
