...
public class FeedbackSyncHelper {
    private static final String TAG = LogUtils.makeLogTag(FeedbackSyncHelper.class);
    ...
    public void sync() {
        ...
        LogUtils.LOGD(TAG, "Number of unsynced feedbacks: " + c.getCount());
        ...
        try {
            while (c.moveToNext()) {
                ...
                if (mFeedbackApiHelper.sendSessionToServer(remoteSessionId, result)) {
                    LogUtils.LOGI(TAG, "Successfully updated session " + remoteSessionId);
                    updatedSessions.add(localSessionId);
                } else {
                    LogUtils.LOGE(TAG, "Couldn't update session " + remoteSessionId);
                }
            }
        } catch (Exception e){
            LogUtils.LOGE(TAG, "Couldn't read from cursor " + e);
        } finally {
            c.close();
        }
