...
public class ImageUpdater {
  ...
  private static final Map<InputJsonKeys.VendorAPISource.MainTypes, String> GCS_IMAGE_FOLDERS;

  static {
    GCS_IMAGE_FOLDERS = new HashMap<>();
    GCS_IMAGE_FOLDERS.put(InputJsonKeys.VendorAPISource.MainTypes.speakers, "images/speakers");
  }
  ...
  public void run(JsonDataSources rawCmsSources) throws IOException {
    ...
    Map<InputJsonKeys.VendorAPISource.MainTypes, List<String>> sourceUrlsByType = parseImageUrls(rawCmsSources);
    for (InputJsonKeys.VendorAPISource.MainTypes type : sourceUrlsByType.keySet()) {
   ...
   */
  static Map<InputJsonKeys.VendorAPISource.MainTypes, List<String>> parseImageUrls(JsonDataSources rawCmsSources) {
    Map<InputJsonKeys.VendorAPISource.MainTypes, List<String>> imageUrls = new HashMap<>();

    // Extract speaker photos.
    imageUrls.put(InputJsonKeys.VendorAPISource.MainTypes.speakers, new ArrayList<String>());
    JsonDataSource speakers = rawCmsSources.getSource(InputJsonKeys.VendorAPISource.MainTypes.speakers.name());
    for (JsonObject speaker : speakers) {
      String photoUrl = speaker.get(InputJsonKeys.VendorAPISource.Speakers.Photo.name()).getAsString();
      if (!isNullOrEmpty(photoUrl)) {
        imageUrls.get(InputJsonKeys.VendorAPISource.MainTypes.speakers).add(photoUrl);
      }
    }
    return imageUrls;
  }
